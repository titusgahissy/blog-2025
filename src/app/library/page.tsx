...
const ResourceCard = ({ book }: { book: Book }) => (
  <div className="flex flex-col gap-4 border-b border-r px-4 md:px-6 py-4 md:py-8 border-neutral-300 dark:border-neutral-700">
    <div className="relative  overflow-hidden shadow-sm h-52 md:h-96">
    ...
    <div >
      <h3 className="font-heading font-semibold text-xl/6 md:text-[1.7rem]/8">{book.title}</h3>
      <div>{book.author}</div>
      ...
      <p className="pt-2 text-xs md:text-sm font-medium leading-relaxed">{book.description}</p>
    </div>
  </div >
)